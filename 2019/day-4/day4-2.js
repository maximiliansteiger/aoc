...
function culcSolution(indexOfNumber, game, finalnumber) {
    ...
    let numbersNow = temparr.slice(0, indexOfNumber + 1);
    let unmarked = 0;
    for (let i = 0; i < game.length - 1; i++) {
        for (let j = 0; j < game[i].length - 1; j++) {
            if (!numbersNow.includes(game[i][j])) {
                unmarked += game[i][j]
            }
        }
    }

    if (indexOfNumber > lastPos) {
        lastPos = indexOfNumber
        finalOutput = unmarked * finalnumber;
    }
}
